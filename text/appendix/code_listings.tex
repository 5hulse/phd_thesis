\chapter{Python Listings}
\label{chap:code-listings}

Presented here are a number of \Python implementations for
the routines described in this text. These can be thought of bare-bones
versions of functions present in \ac{EsPy} package. These listings require
\Python version 3.8 or higher, and also require the \texttt{numpy} and
\texttt{scipy} packages to be installed from the \Python Package Index.
...
applied to \ac{1D} estimation, as well as the gradient and Hessian.
}{
Code for the generation of the fidelity for \ac{NLP}
applied to \ac{1D} estimation, as well as the gradient and
...
\ac{1D} estimation, with an approximated Hessian.
For the equivalent code for both \ac{2D} estimation, and for computing the
exact Hessian, the reader is directed to \protect\path{nmrespy/nlp/_funcs.py} in the
\ac{EsPy} package.
}{lst:obj-grad-hess}
