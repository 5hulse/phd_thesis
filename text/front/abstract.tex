\begin{abstract}
    ...
    dynamics of chemical species. To maximise the utility of \acf{NMR}
    ...
    experimental data, the \acfi{FID}, is to convert it to an \acs{NMR}
    ...
    simplicity of the representation provided, the \acs{FT} suffers from poor
    ...
    for the analysis of \acfp{FID} derived from solution-state \acs{NMR}
    ...
    for the generation of pure shift \acs{NMR} spectra with desirable properties
    from \acl{2DJ} datasets. Other applications presented here are for the
    analysis of \acs{2D} datasets in which each \acs{FID} exhibits a variation
    in its amplitude --- including inversion recovery ($T_1$), \acs{CPMG}
    ($T_2$) and diffusion experiments ---, and a means of producing phased,
    ultra-broadband \acs{NMR} spectra from an experiment comprising a single
