...
\nomenclature[A,03]{$x \propto y$}{$x$ is proportional to $y$.}
...
\nomenclature[A,07]{$a \bmod b$}{$a$ modulo $b$, given by \[a \bmod b = a - \left\lfloor \frac{a}{b} \right\rfloor b.\]}

%%% Sets
\nomenclature[B,01]{$x \in X$}{$x$ is a member of the set $X$.}
\nomenclature[B,02]{$X \subset Y$}{The set $X$ is a subset of the set $Y$.}
\nomenclature[B,03]{$\mathbb{C}$}{The set of complex numbers.}%
\nomenclature[B,04]{$\mathbb{N}$}{The set of natural numbers with zero excluded.}%
\nomenclature[B,05]{$\mathbb{N}_0$}{The set of natural numbers with zero included.}%
\nomenclature[B,06]{$\mathbb{R}$}{The set of real numbers.}%
\nomenclature[B,07]{$\mathbb{R}_{>0}$}{The set of positive real numbers.}%
\nomenclature[B,08]{$\mathbb{R}_{>1}$}{The set of real numbers greater than one.}%
\nomenclature[B,09]{$\mathbb{F}$}{A field (i.e. $\mathbb{R}$, $\mathbb{C}$, etc.).}
\nomenclature[B,10]{$\lbrace x, \cdots, y \rbrace$}{The set of consecutive
integers from $x \in \mathbb{Z}$ to $y \in \mathbb{Z}$ (both included).}
\nomenclature[B,11]{$(x, y)$}{The range of numbers from $x \in \mathbb{R}$ to $y \in \mathbb{R}$ (both excluded).}
\nomenclature[B,12]{$[x, y]$}{The range of numbers from $x \in \mathbb{R}$ to
$y \in \mathbb{R}$ (both included).}
\nomenclature[B,13]{$(x, y]$}{The range of numbers from $x \in \mathbb{R}$
(excluded) to $y \in \mathbb{R}$ (included).\\}
...
\nomenclature[D,01]{$\symbf{x}$, $\symbf{X}, x_n$}{
    ...
    row and third column of a matrix $\symbf{Y}$ would be denoted $y_{\hspace{1pt}2,3}$. In
    some circumstances where it is appropriate, indexing begins at 0
    rather than 1.
}
...
\[
    ...
    \sum_{n_D = 1}^{N_D}
    x_{n_1, \cdots, n_D}^*\hspace{1pt}y_{n_1, \cdots, n_D}.
\]
}
\nomenclature[D,07]{$\lVert \symbf{X} \rVert$}{Norm of an array, given by
    \[
...
\nomenclature[D,08]{$\symbf{x} \cdot \symbf{y}$}{
    ...
    \mathbb{F}^N$, which generates a scalar $a$ given by
    \[
...
\nomenclature[D,11]{$\symbf{x}[a:b]$}{
    Slicing notation, following the
    convention used by \textsc{NumPy}~\cite{indexing}. For $\symbf{x} \in
    \mathbb{F}^N$, $a \in
    \lbrace 0, \cdots, N-1 \rbrace$, and $b \in \lbrace a+1, \cdots, N \rbrace$,
    the slice $\symbf{x}[a:b]$ produces a vector $\in \mathbb{F}^{b-a}$ comprising
    the consecutive elements of $\symbf{x}$ from the ($a +
    1$)\textsuperscript{th} to the $b$\textsuperscript{th}. This can be
    extended for an array of any dimension, e.g. $\symbf{X}[a_r : b_r, a_c :
    b_c]$ denotes the slice of a 2D matrix, with the row slice denoted first,
    followed by the column slice.
}
...
\nomenclature[E,03]{$\mathcal{U}\left(l, r\right)$}{
...
\nomenclature[E,04]{$\mathcal{N}\left(\mu, \sigma^2\right)$}{
...
\nomenclature[E,05]{$\mathcal{N_C}\left(\mu, \sigma^2\right)$}{Complex normal distribution with mean $\mu$ and variance $\nicefrac{\sigma^2}{2}$.}%

%%% Calculus
\nomenclature[F,01]{$\nabla f(\symbf{x})$}{
    Gradient vector of a differentiable, scalar function $f(\symbf{x}):
...
\nomenclature[F,02]{$\nabla^2 f(\symbf{x})$}{
    Hessian matrix of a twice-differentiable, scalar function
