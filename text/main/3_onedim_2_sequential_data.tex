...
\cref{fig:five-multiplets-invrec}.d (\cpageref{fig:five-multiplets-invrec}).

\subsubsection{Measuring $T_2$: \acs{CPMG}} \label{subsec:cpmg}
...
\correction{
    $\Updelta$ is the time from the start of the first \ac{PFG}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State $\bth_{1}, \symbf{\epsilon}_{1} \gets \textsc{Estimate$1$D}\left(
            ...
            \right)
        $;
        \Comment{Estimate first increment, see \cref{alg:1d-2d-summary} (\cpageref{alg:1d-2d-summary}).}
        ...
        \For{$k=2, \cdots, K$}
            \State $\widetilde{\by} \gets \textsc{Filter$1$D}\left(
                ...
                \right);
                $
            \Comment{\cref{alg:filter-1d} (\cpageref{alg:filter-1d}).}
            \State $\bth_{k}, \symbf{\epsilon}_{k} \gets
            \textsc{NLPAmp}\left(\widetilde{\by}, \bth_{k-1}\right)$;
            \Comment{Similar to \cref{alg:nlp} (\cpageref{alg:nlp}), though see the caption.}
...
\correction{
    ...
    provided in \cref{subsec:errors} (\cpageref{subsec:errors}).
...
\cpageref{subsec:invrec-datasets} for more
information).
...
through proton exchange with the residual water\footnote{
...
\correction{
    The small but apparent difference in diffusion constants associated with
    the anomers is likely related to how effectively they are able to
