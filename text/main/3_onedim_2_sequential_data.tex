...
opposite polarity to the first. The \ac{PGSTEBP} experiment
(\cref{fig:diffusion_sequences}.c) is analogous \ac{PGSTE}, with monopolar
gradients replaced with bipolar ones. Bipolar gradients are useful in
circumstances where it is important to purge the effects of static gradients in
the sample, caused by field inhomogeneities. Morris and coworkers have
developed the \emph{one-shot} experiment\cite{Pelta2002}
...
The most common approach to conducting a diffusion experiment is to run an
...
\begin{subequations}
    \begin{alignat}{2}
        & \text{Monopolar gradients (\ac{PGSE}, \ac{PGSTE})} \quad && \Updelta + 2 \left(\kappa - \lambda\right) \delta,
        \label{eq:monopolar}\\
        & \text{Bipolar gradients (\ac{PGSTEBP})} \quad && \Updelta + \frac{\left(2 \kappa - 2 \lambda
            - 1\right)\delta}{4} - \frac{\tau}{2},\\
        & \text{One-shot}
            \quad && \Updelta + \frac{\left(\kappa - \lambda\right)
...
\nicefrac{1}{2}$, and  $\kappa = \frac{1}{3}$. In many situations,  $\Updelta$
...
\end{figure}
\Cref{fig:five-multiplets-invrec} shows the result of the described
