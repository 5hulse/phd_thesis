...
\correction{The errors in the parameter of interest $\symbf{\epsilon}$ can be extracted
    from the Hessian matrix at convergence; more detail on the errors from
    \ac{NLP} this is provided in \cref{subsec:errors}, in the context of \ac{FID}
    estimation.}\label{corr:seq-errors}
\begin{equation}
...
\end{equation}
        % \lVert
        %     \symbf{a}_m - a_0 \mathcal{A}(\psi_m^{(*)} \vert \symbf{p})
        % \rVert^2
