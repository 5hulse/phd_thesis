...
For each \ac{FID}, the \ac{MPM} routine outlined in \cref{lst:mpm} (\cpageref{lst:mpm}) was
performed 5 times.
...
implementation used (\cref{lst:mmempm}, \cpageref{lst:mmempm}) employs a
...
\begin{enumerate}
    \item Generation of all the first partial derivatives
        (\cref{eq:first-derivs}, \cpageref{eq:first-derivs}), which are stored
    ...
    \item Generation of all the non-trivially zero second partial
        derivatives (\cref{eq:second-derivs}, \cpageref{eq:second-derivs}),
