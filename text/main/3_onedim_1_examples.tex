...
frequencies, causing considerable peak overlap in the Fourier domain, and
low \ac{SNR}. To assess the ability of the inclusion of the
phase variance-regularised \ac{NLP} routine to produce parameter estimates
in better agreement with known features about the data,
...
converted back to the time-domain using \ac{IFT}.
...
\begin{sidewaysfigure}
    ...
    \caption[
        ...
        initial guess.  Also included in \textbf{c.} and \textbf{d.} is the
        residual between the data and the sum of the oscillator peaks (grey
        line). The colouring of the oscillators in panels c and d is described
        in the main text.
    }
...
\ac{SNR} of \qty{25}{\deci\bel}, with the corresponding noise variance for each
\ac{FID} determined using the equation
\begin{equation}
...
\cref{fig:mpm_vs_nlp}, with the set of oscillator peaks which contribute
...
result to \ac{NLP}, oscillators which satisfied $a_m < 0.1$ and/or  $\eta_m
...
In panel c of \cref{fig:mpm_vs_nlp}, blue oscillators are those which
...
non-blue oscillator parameters in panel c such that they agree with true
signals, while having little to no effect on the blue oscillators. The results
after application of \ac{NLP} are provided in panel d.
...
albeit with parameters which are noticeably deviate from the true signals are
...
such that it was removed from the parameter set. This typically occurred with
oscillators that had an initial phase which was either $>
\nicefrac{\pi}{2}$\,\unit{\radian}, or $< -\nicefrac{\pi}{2}$\,\unit{\radian}.
Yellow oscillators therefore indicate cases where the final result
...
denoted by an asterisk in panel a of \cref{fig:mpm_vs_nlp}.
...
\begin{sidewaysfigure}
    ...
    \caption[
        ...
        the model order predicted with the \acs{MDL}. Blue/red lines:
        Individual oscillator peaks, grey line above: the model (sum of all
        oscillators), grey line below: the residual between the data and the model.
...
\ac{MPM} are presented in panel b of \cref{fig:andro-onedim}. The initial guess
was then subjected to \ac{NLP}, giving rise to the result in panel c.
...
spin reasonably well. \Cref{tab:andro-multiplets} provides an overview of the
...
common presence of signals within the dataset that have incredibly similar
...
more bonds) alongside the ubiquitous two-bond (geminal) and three-bond
...
underfit of the multiplet structure.
...
exceptionally good fits can be achieved using the exponentially-damped
sinusoids model.
...
accurately parametrised by 8 oscillators in the model; the \ac{NLP} routine
...
with signals corresponding to spins (E) and (F).
...
\textdagger in panels b \& c of the figure) acquire amplitudes which
...
with an unexpected ratio of $\approx 1:1$. It is not surprising
that greater deviations from expected behaviour occur in more heavily crowded spectral
