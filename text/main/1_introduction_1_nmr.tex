...
that $\symbf{W}$ is an array of \ac{AWGN}. This means that the noise instances
are assumed to be described by a complex normal distribution with mean 0, and
...
the signal comprises. Due to he large dynamic range that the \ac{SNR} can
exhibit, it is commonplace to express it using a logarithmic scale instead, in
decibels (\unit{\deci\bel}):
\begin{equation}
    \begin{split}
        \SNR_{\unit{\deci\bel}} &\coloneq 10 \log_{10} \left(\SNR\right).
    \end{split}
\end{equation}
From this, the noise variance associated with a signal a given
$\SNR_{\unit{\deci\bel}}$ is
\begin{equation}
    \sigma^2 = \frac{1}{ 20^{\nicefrac{\SNR_{\unit{\deci\bel}}}{10}}\mathfrak{N}}
        \sum_{\none=0}^{\None-1} \cdots \sum_{\nd=0}^{\ND-1}
        \left \lvert \bX \left[\none, \cdots, \nD \right] \right \rvert^2
\end{equation}
