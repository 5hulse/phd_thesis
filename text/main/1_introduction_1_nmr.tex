...
\input{text/generated/gyromagnetic_ratios.tex}
...
magnitude $M_0$.
\correction{
    The Boltzmann distribution describes the relative populations of the spin states:
    \begin{equation}
        p_{m} = \frac{\exp\left(\frac{m\hbar \gamma B_0}{k_{\text{B}} T}\right)}
        {\sum\limits_{m=-I}^I \exp\left(\frac{m\hbar \gamma B_0}{k_{\text{B}} T}\right)},
        \label{eq:boltzmann}
    \end{equation}
    ...
    to be satisfied at all temperatures used in
    solution-state \ac{NMR}. For this reason, the exponential terms in
    \cref{eq:boltzmann} may be approximated using a Taylor expansion truncated
    at the first-order term.
...
\correction{
\ac{NMR} experiments employ \ac{RF} pulses which are linearly polarised
and modulated cosinusoidally. As an example, a pulse which is polarised along
