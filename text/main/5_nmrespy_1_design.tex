...
\begin{itemize}
    ...
    \item The \textsc{SciPy} ecosystem\cite{Virtanen2020}, including the packages
        ...
        high-performance scientific computation and visualisation in
    ...
    \item Being a scripting language with user-friendly syntax makes
        ...
        estimation routine,
        (c) output and inspect the result.
    ...
    \ac{NMR} data types in mind (see \ref{subsec:estimator-objects}).
\end{itemize}
...
\emph{estimators}.
Instances of these estimator objects contain relevant piece of data called
...
\begin{itemize}
    \item \textbf{\textbf{\texttt{ExpInfo}}} stores the parameters used for a
        particular \ac{NMR} experiment. \texttt{ExpInfo} also has methods for
        the generation of the timepoints/chemical shifts sampled based on these
        parameters, and for producing synthetic \acp{FID}.
    \item \textbf{\texttt{\_Estimator}} inherits experimental parameters from
        ...
        an object is often referred to as an abstract class). For child
    ...
    \item \textbf{\texttt{\_EstimatorSeqAmp}} is an abstract class
        ...
        which that data should be imported/generated varies. As such, this is not
        ...
        number of \texttt{Estimator1D} objects, with each object containing one
...
\texttt{EstimatorCPMG} would enable the analysis of \ac{CPMG} datasets,
...
implementing \ac{2D} and \ac{3D} estimators is given in \cref{sec:future-work}.

\subsection{Example usage}
\mylisting{python3}{code_listings/cupid_example.py}{%
    An example script which makes use of \acs{EsPy}.
}{
    An example script which makes use of \acs{EsPy} in order to make use of the
    \ac{CUPID}.
}{lst:espy-example}
...
\begin{enumerate}
    \item An \texttt{Estimator2DJ} object is initialised to import and store
        the dataset, stored in \textsc{Bruker} format at the path
    ...
    \item After estimation is complete, the estimator object is saved to a byte
        stream using \Python's ``pickling'' protocol\cite{pickle} (Line
        \ref{ln:pickle}).
    ...
    \item A pure shift spectrum is produced via the \ang{-45} signal, and
        ...
        \ref{ln:writecupid}).
    \item Finally, a PDF figure depicting the estimation result is produced and
