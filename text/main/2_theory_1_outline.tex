...
\ac{NLP} do exist,\note{(cite examples like simplex, simulated annealing,
...
the description of \ac{VARPRO} and \ac{AMARES} in
\cref{subsec:estimation-techniques}). As an
alternative, a routine is presented here in which an initial guess is
determined using \iac{SVD}-based technique (the \ac{MPM}), requiring no or
minimal user input. Subsequently, the result generated is subjected to \ac{NLP}
under the assumption that the data is phase-corrected, which will be shown to
be effective in correcting spurious features in \ac{MPM} result. In this sense,
the \ac{NLP} routine can be seen as a means of validating the \ac{MPM} result,
and correcting spurious features, if necessary.
