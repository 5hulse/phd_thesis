...
To gain insights from \ac{NMR} experiments on a chemical system of interest,
extraction of the defining parameters $\bth$ is necessary. In this section, the
conventional approach to processing and analysing \ac{NMR} data is described.
Following this, some of the more prevalent methods for parametrising \ac{NMR}
\acp{FID} are presented.

\subsection{Conventional NMR Processing}
\label{subsec:nmr-analysis}
The typical means of processing \ac{NMR} experiments for analysis is to
transform the time domain \ac{FID} into the frequency domain, producing an
...
\begin{subequations}
    \begin{align}
        &\text{Continuous case:} &&\FT(x(t))(F) =  \int_{0}^{\infty} x(t) \exp(
            ...
            \quad \forall t \in \mathbb{R}_{\geq 0},\\
...
takes the form of a \emph{Lorentzian}\footnote{
    In practice, when dealing with discrete data, the \ac{FT} of an \ac{FID}
...
either the sensitivity or the resolution of the final spectrum. This does come
at the cost of worsening the other feature however.
...
(see panel d of Figure \ref{fig:amp-phase-freq-damp}).
...
cosmetic improvement to the spectrum, but it does not incorporate any new
information into it.
...
absorption Lorentzians is therefore desired, which is possible if all signal
...
There a number of potential causes of baseline distortion,
...
This involves multiplying the spectrum with a high-order polynomial function,
whose coefficients are determined by an appropriate algorithm. A popular
algorithm involves the two-step procedure of (a) determining spectral regions
which are part of the baseline (b) fitting the baseline regions to a
...
absorption-mode lineshapes in each dimension.
 A description of how this can be achieved with both
amplitude- and phase-modulated datasets is now given.
...
each dimension, the requisite pair of amplitude- or phase-modulated signals
exist. As such, a set of $2^{D-1}$ signals need to be acquired.
...
The concept of \ac{LP} stems from the idea that a deterministic signal like a
...
Some of the most prevalent methods for \ac{NMR} estimation involve \acfi{SVD}
as a key component (\S \ref{subsec:linear-algebra}). For each of these methods,
...
parameters in \eqref{eq:x} (frequencies and damping factors, incorporated in
the signal poles).
...
\ac{MMEMPM}\cite{Hua1992,Chen2007} are provided in detail in \S
\ref{subsec:mpm}.
...
% coworkers\cite{Barkhuijsen1985a,Barkhuijsen1985b}. Given the backward \ac{LP}
...
% \eqref{eq:lineqs} implies that $\symbf{b}$ can be determined by employing
% linear lest squares:
% \begin{equation}
...
% low-rank approximations of matrices can found found in \S \ref{subsec:mpm} and
% \S \ref{subsec:linear-algebra}.
...
% right and left singular vectors, respectively. As will be illustrated in \S
% \ref{subsec:mpm}, the complex amplitudes associated with the \ac{FID} can be
...
recast as $\symbf{Z}^+\symbf{y}$ (see \eqref{eq:complex-amplitudes}). By doing
...
\note{Mention deconvolution networks.}
