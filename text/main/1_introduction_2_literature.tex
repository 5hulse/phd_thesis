...
be described as a linear combination of a certain number of previous samples:
\begin{equation}
...
It was described in Section \ref{subsec:ar_nmr} that noiseless NMR signals
obeying Eq. \ref{eq:general_x} are AR($M$) processes, while those corrupted
with noise (Eq. \ref{eq:general_y}) are ARMA($M$,$M$) processes. Autoregressive
methods applied to NMR data tend to assume the data is described by the more
simple AR($M$) model despite the fact that it truly is an ARMA process. A
popular method of extracting the underlying signal parameters
$\left\lbrace b_k \right\rbrace $
is application of the Yule-Walker method\cite{Yule1927,Walker1931}, an approach
based on the autocorrelation coefficients of the data, which are approximated
as
\begin{equation}
    R_i \equiv R_{-i}^* \approx \frac{1}{N-i} \sum_{n=0}^{N-i-1} y[n+i]^{\vphantom{*}} y[n]^*.
\end{equation}
Multiplying both sides of Eq. \ref{eq:forward_ar} by $y[n-i]^*$, and taking the
expectation value, leads to:
\begin{subequations}
    \begin{gather}
        \begin{gathered}
            \mathbb{E}\left[ y[n] y[n-i]^* \right] =
            \mathbb{E}\left[ -\sum_{k=1}^M b_m y[n-k] y[n-i]^* \right] +
            \mathbb{E}\left[ w[n]y[n-i]^* \right]\\
            \implies R_i = -\sum_{k=1}^M b_k R_{i-k} + \delta_i
        \end{gathered}\\
        \delta_i =
        \begin{cases}
            \sigma^2 & i = 0\\
    ...
    \end{gather}
\end{subequations}
Expressing this in matrix-vector format, one arrives at
\begin{equation}
    \begin{bmatrix}
        R_0 & R_{-1} & \cdots & R_{-M} \\
        R_1 & R_0 & \cdots & R_{1-M} \\
        \vdots & \vdots & \ddots & \vdots \\
        R_M & R_{M-1} & \cdots & R_0 \\
    \end{bmatrix}
    \begin{bmatrix}
        1 \\ b_1 \\ \vdots \\ b_M
    \end{bmatrix}
    ...
    \end{bmatrix}.
\end{equation}
The unknowns of this equation are the coefficients
$\left\lbrace b_k \right\rbrace $
and $\sigma^2$. Possibly the most easy-to-envisage approach to solving this is to compute the
$\left\lbrace b_k \right\rbrace $s first via
\begin{equation}
    \begin{bmatrix}
        b_1 \\ \vdots \\ b_M
    \end{bmatrix}
    ...
    \end{bmatrix},
\end{equation}
and then to compute $\sigma^2$:
\begin{equation}
    \sigma^2 = R_0 + \sum_{k=1}^M b_k R_k.
\end{equation}
Computation of the inverse of the autocorrelation matrix via Gaussian
elimination has an arithmetic complexity of $\mathcal{O}\left(n^3\right)$.  A
more efficient approach is to use the Levinson-Durbin
algorithm\cite{Levinson1946,Durbin1960} which exploits the Toeplitz structure
of the matrix and runs in $\mathcal{O}\left(n^2\right)$ time.
