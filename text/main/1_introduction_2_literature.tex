...
\correction{
    While not the optimal choice\footnote{
        \correction{
            ...
            window, which minimises the magnitude of truncation artefacts for a given
...
\iac{FID}. The Lorentz-Gauss function bestows a Gaussian shape to spectral peaks,
\correction{which features a narrower base relative to an equivalent
Lorentzian.}\label{corr:gaussian} Another popular window
...
truncated \ac{FID} is akin to the convolution of the spectrum of its untruncated
...
form of sinc functions ($\operatorname{sinc}(x) \coloneq \nicefrac{\sin(x)}{x}$).
