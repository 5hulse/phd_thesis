...
the \ac{FT}. The \ac{FT} converts a continuous function over time $t$, to one
...
halving the initial point of the \ac{FID} prior to \ac{DFT}.
...
series of Lorentzian ``peaks'', located at the frequencies of the signals
in the \ac{FID}:
\begin{equation}
...
becomes more
weighted towards the noise, as spin relaxation phenomena attenuate the signal.
Multiplying the \ac{FID} with a function which decreases with time
can be used to enhance the \ac{SNR} of the \ac{FID}. The most common function
to achieve this is the negative exponential,
...
the untruncated \ac{FID}, with the \ac{FT} of a box-function, taking the
...
dispersive components, and also also from significant non-linearities. In scenarios
...
to estimate the optimal set of parameters $\bth$ which describe them\footnote{
    ...
    signal by the process of summing a number of complex sinusoids, though it
    is often described as so in the \ac{NMR} literature.
}.
...
The concept of \ac{LP} stems from the idea that \iac{FID} can be described as
...
There is not much precedent for using iterative schemes for \ac{NMR} analysis,
...
common nucleus studies in \ac{MRS}, the number of potential species
...
\textit{a priori} information for each dataset of interest may become
time-consuming and cumbersome. There is also a good possibility that the
expected identity of the dataset in unknown in \ac{NMR}, making any method
which requires intimate knowledge about the data worthless.
...
model until a maximisation of the posterior probably is achieved, such that
both model order selection and parameter estimation are achieved
simultaneously. In order to reduce the computational burden of the
estimation, the complete \ac{FID} is downsampled into a series of
...
indirect-dimension is estimated separately, yielding a $4 \times M$ parameter
set for each indirect increment. Such an approach is not able to extract all $6
\times M$ parameters associated with the \ac{2D} dataset however.

\subsubsection{Machine Learning Methods}
The vast surge in prevalence of machine learning across virtually all
scientific disciplines has unsurprisingly lead to work which applies it for the
purposed of \ac{NMR} estimation in recent years. At the time of writing,
approaches have focussed on Fourier-domain estimation\cite{Li2021,Schmid2023}.
The currently available methods employ deep (i.e. many-layered) neural
networks, featuring numerous deconvolution layers, which have been shown to be
widely successful in image- and signal processing applications.
