...
Once a provisional update $\bthkplusone = \bthk + \bpk$ is determined, a
metric is considered which indicates how effectively the quadratic estimate at
update agrees with the true value of the fidelity at this point:
\begin{equation}
    \rho^{(k)} = \frac
        {\Fthk - \Fthkpk}
        {\FQthk - \FQthkpk}.
...
they are $0$ by definition. See \cref{tab:number-of-derivatives} for the total
number of derivatives that need to be computed for signals with different
...
\begin{table}
    \begin{center}
        \begin{tabular}{ c c c }
            ...
            $D$ &  $2(1 + D)M \None \cdots \ND$ &  $((1 + D)(3 + 2D) - 1)M \None\cdots\ND$\\
            \bottomrule
        \end{tabular}
    \end{center}
    \caption{
...
In both cases, the \ac{NLP} routine successfully converged at result $\bthstar$
...
approximated analogue (red), which exhibited ``zig-zagging''\footnote{
    ...
    update occurs in the opposite direction to the gradient.
}.
...
model order is used. However, with \ac{FID} estimation being a ill-posed
problem,(\note{details/citation?}) there are conceivably a large subset of
parameter vectors which will achieve this. It is desirable to produce an
estimate which not only achieves a good fit to the data, but which is also able
to explain the process underpinning the observation. It is for this reason that
iterative procedures typically require significant quantities of \textit{a
priori} information in order to produce meaningful estimation results (see the
discussion of \ac{VARPRO} and \ac{AMARES},
\cref{subsec:estimation-techniques}).

While the \ac{MPM} is often able to generate reasonable parameter estimates,
one particular feature has been noticed which is systemic across results; it is
common for the oscillators which comprise the model to exhibit \emph{spurious
phases}. As has been discussed (see \cref{subsec:nmr-proc}), the phases of
...
acquire a phase of approximately \ang{0}. This feature of the dataset can be
...
the data has been phase corrected, incorporating the variance of oscillator
phases into the fidelity can lead to improved estimation results, examples of
which will be provided subsequently (\cref{sec:evaluation}).
