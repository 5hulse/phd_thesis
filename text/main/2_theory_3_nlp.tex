...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Procedure {NLP}{$\bY \in \mathbb{C}^{\None \times \cdots \times \ND}, \bthzero \in \mathbb{R}^{2(D + 1)M}$}
            ...
            \Comment{See \cref{subsec:errors}.}
...
was added to the \ac{FID} to give it a target \iac{SNR} of
...
\begin{remark}
    \correction{
        Whenever simulated \ac{NMR} data is corrupted with noise in later examples
        in this thesis, the type of noise is always \ac{AWGN}.
    }\label{rem:awgn}
\end{remark}
