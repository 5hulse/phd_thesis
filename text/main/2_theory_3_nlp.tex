...
The structure of a trust region method is presented in Algorithm
...
\mathbb{R}^{2(D+2)M} \rightarrow \mathbb{R}$ is given by
\begin{equation}
...
\end{remark}
The elements of the gradient vector $\bdgthY \in \mathbb{R}^{2(D+2)M}$ and
the Hessian matrix $\bdHthY \in \mathbb{R}^{2(D+2)M \times 2(D+2)M}$ are then
...
\end{subequations}
$\forall i,j \in \lbrace 0, \cdots, 2(D + 2)M - 1 \rbrace$.
The complete set of first derivatives of a particular element of the model $x
\coloneq \bXelem$ (see Equation \ref{eq:x}) are as follows $\forall m \in
...
\begin{subequations}
    ...
    \end{gather}
\end{subequations}
The complete set of the second derivatives are given by
\begin{subequations}
    \begin{gather}
    ...
    \begin{algorithmic}[1]
        \Procedure {NLP}{$\bY \in \mathbb{C}^{\None \times \cdots \times \ND}, \bthzero \in \mathbb{R}^{2(D + 1)M}$}
            ...
            \For {$k = 0, 1, \cdots $}
                ...
                \Comment{See Algorithm \ref{alg:cg-steihaug}}
                \State $\rho^{(k)} \gets
                    \frac
                        {\Fphithk - \Fphithkpk}
                        {\FphiQthk - \FphiQthkpk}$;
                ...
                \ElsIf {$\rho_k > \nicefrac{3}{4}$ \textbf{ and } $\left\lVert \symbf{p}^{(k)} \right\rVert = \Delta^{(k)}$}
