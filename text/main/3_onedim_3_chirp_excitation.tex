...
\correction{
    Further to methods involving chirp pulses, optimal control
    theory~\cite{Skinner2003} has yielded effective pulses for broadband
    excitation and inversion, albeit across narrower bandwidths, with
    \ac{BEBOP} being a prominent example~\cite{Skinner2004,Kobzar2012}.
...
\begin{figure}
    ...
    {
        An illustration of an experiment comprising a single chirp pulse \correction{
            of nominal flip angle \ang{90},
        }\label{corr:ninty-flip} which sweeps from
...
\begin{figure}
    ...
    ]
    {
        ...
        (\cref{eq:spec-phase-chili}). \correction{
            Baseline distortions are most prominent in the downfield (i.e. low-frequency) region of the spectrum.
        }\label{corr:baseline-dist}
...
\correction{
    For datasets with closely-spaced signals (i.e. where J-couplings lead to
    multiplet structures) such a severe truncation of the data may not be
    feasible, since the individual signals will not have evolved for sufficient
    time to be resolvable. In such cases, a less truncated \ac{FID} would need
    to be considered, at greater computational cost. Alternatively, the data
    could be filtered to form appropriate sub-\acp{FID}, as outlined in
    \cref{sec:filtering}. The efficacy of the filtering method for this type of
    data may not be sub-optimal, as slowly decaying peaks with dispersion
    character will be feature.
...
\correction{
    1\% \ch{H2O} by volume in a solution
    comprising \qty{0.1}{\milli\gram} \ch{GdCl3}/\qty{1}{\milli\litre}
    \ch{D2O}\footnote{
        The paramagnetic species \ch{Gd^{III}} is a popular contrast agent used
        in \ac{MRI}, owing to its ability to decrease the $T_1$ of nearby
        spins. This typically has an influence on $T_2$ too, with it being
        shortened at high enough concentrations.  The signal from \ch{H2O}
        therefore decays at a more rapid rate, such that the baseline
        distortions observed in the phased spectrum of
...
\begin{figure}
    ...
    \caption[
        ...
        \correction{
            The data were acquired using a \ac{2D} experiment in which the
            transmitter offset was varied across 13 increments. For each increment, $N
            = 2^{17}$ and $\fsw = \qty{394.7}{\kilo\hertz}$.
            The transmitter offsets took the values $\foff =
            \qty{-21.8}{\kilo\hertz}$ to $\foff = \qty{-273.8}{\kilo\hertz}$,
            in equal steps of \qty{-21}{\kilo\hertz}. 4 transients were acquired
            per increment. The 13 increments were then summed to produce
            the final \ac{FID}.%
