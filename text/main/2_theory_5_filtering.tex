...
\correction{
    Every result which involved filtering made use of the process
    employed above. However in retrospect, a more straightforward method
    of applying a rectangular filter, and slicing the filtered spectrum exactly
    at the filter boundaries would likely yield sub-\acp{FID} with the
    same information content, without the requirement to incorporate synthetic
    noise into the filtered dataset.
