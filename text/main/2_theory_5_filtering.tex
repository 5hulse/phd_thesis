...
(ii) applying a band-pass filter to the spectral region of interest, and
(iii) returning the spectrum back to the time-domain for estimation.
...
region\footnote{
    Lorentzian lineshapes tend to but never explicitly reach zero as the distance from
    their maximum tends to $\infty$ (\cref{eq:lorentzian}). However, as long as a
    sufficiently wide filter region is defined, the extent to which the
    ``tails'' of the Lorentzian fall outside the filter window can be assumed
    to be negligible, especially when in the presence of noise.
}.
