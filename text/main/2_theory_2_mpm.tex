...
associated with noise. Assuming the \ac{SNR} of \iac{FID} is sufficiently high,
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Procedure{MPM}{$\by \in \mathbb{C}^{N}, \fsw \in \mathbb{R}_{>0}, \foff \in \mathbb{R}, M \in \mathbb{N}$}
...
determined using iterative approaches like the Arnoldi
...
diagonal of $\symbf{G}$ (see panels a1 \& a2 of \cref{fig:mmempm-poles}).
...
the form, allowing for reorganisation of rows and columns:
\begin{equation}
    \symbf{G} =
    \begin{bmatrix}
...
panels b1 \& b2 of \cref{fig:mmempm-poles}). Note that $\sum_{r=1}^R h_r = M$.
When $h_r > 1$, the subset of poles $\bdztwo_r$ are determined by computing the
...
leads to the model which also features influence from the noise. The penalising
term $\mathcal{P}(k)$, which is always increasing, is required in order to
...
the \ac{FID}'s model order when the \ac{SNR} is higher (panel c).
