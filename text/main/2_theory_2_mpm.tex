...
\begin{subequations}
   ...
   \end{gather}%
    \label{eq:HX-decomp}%
\end{subequations}%
The matrix pencil\footnote{
        \correction{
            For two matrices $\symbf{A},
            \symbf{B} \in \mathbb{C}^{m \times n}$, their \emph{pencil} is the
            set of all matrices of the form $\symbf{A} - \lambda \symbf{B}$,
            with $\lambda \in \mathbb{C}$~\cite[Section 7.7]{Golub2013}. In
            most contexts, the matrices involved are assumed to be square
            (i.e. $m = n$); this is not the case in the context of the \ac{MPM}
            however.
        }\label{fn:matrix-pencil}
} $\Hxtwo - \lambda\Hxone$, with $\lambda \in
...
\lambda \symbf{I}_M]_{m,m}$ becomes $0$, which will lead to the matrix pencil's
rank being reduced by $1$, i.e. the signal poles are the rank-reducing values
of the matrix pencil.
One means of extracting these poles is by computing the eigenvalues of the
...
\cref{lst:mpm} (\cpageref{lst:mpm}) provides an example \Python implementation.
For optimal results, the pencil parameter should adhere to $\lfloor
\nicefrac{N}{3} \rfloor \leq L \leq
...
determined using iterative approaches like the Rayleigh-Ritz
...
\begin{figure}
    ...
    \caption[
        The structure of the matrix $\symbf{G}$ from the \acs{MMEMPM}, for the
        cases of an \acs{FID} with distinct and repeated signal poles in the
        indirect dimension.
    ]{
        The structure of the matrix $\symbf{G}$ from the
...
$\bdzone$\footnote{
    ...
    frequencies satisfy
    \[
        \lvert \fone_i - \fone_j \rvert < \nicefrac{\fswone}{\None}
    \]
    (recall \cref{eq:poles-to-f}, which states the
...
See \cref{alg:mmempm} (\cpageref{alg:mmempm}) for a pseudo-code outline, and
\cref{lst:mmempm} (\cpageref{lst:mmempm}) for a \Python implementation of the
\ac{MMEMPM}.
...
constitute more than one signal, leading to under-estimates of $M$.
...
of potential models comprise \cref{eq:x} (\cpageref{eq:x}) with variable $M$.
Both the \ac{AIC} and \ac{MDL} take the same general form:
\begin{equation}
    \mathcal{C}(k) = -c \ln \left(\Lbdthstary\right) +
    \mathcal{P}(k) \text{ with } \bthstar \in \mathbb{R}^{4k}
    \quad \forall k \in \lbrace 0, 1, \cdots \rbrace.
\end{equation}
$\Lbdthstary$ is the likelihood
...
\begin{equation}
    \mathcal{L}\left(\bthstar \in \mathbb{R}^{4k} | \hspace{2pt} \by\right) = \left(
        \frac{
            \prod_{r=k}^{L-1} \sigma_{r+1}^{\nicefrac{1}{L-k}}
        }{
            \frac{1}{L-k} \sum_{r=k}^{L-1} \sigma_{r+1}
...
\begin{subequations}
    \begin{gather}
        \operatorname{AIC}(k) = -2 \ln\left( \Lbdthstary \right) + 2k(2 L - k), \\
        \operatorname{MDL}(k) = -\ln\left( \Lbdthstary \right) + \tfrac{1}{2} k(2 L - k) \ln N. \label{eq:mdl}
    \end{gather}
\end{subequations}
...
applications, and is employed in this work\footnote{
    ...
    first relative minimum is determined (see \cref{state:mdl} of
    \cref{alg:mpm}, \cpageref{alg:mpm}). When $k$ is very large, it has been
    observed that the \ac{MDL} can spuriously jump in value. This often occurs
    when \acp{FID} with very high \acp{SNR} are analysed; the singular values
    involved can tend so closely to zero that issues related to the instability
    of floating-point arithmetic manifest.  \label{fn:argrelmin}
}:
\begin{equation}%
...
\begin{figure}%
    ...
    \caption[%
        The behaviour of the \acs{MDL} for three different
        ...
        85$.
        \textbf{c.} Square points with dotted lines: The negative log-likelihood
        ...
        \qty{20}{\deci\bel}
        \acs{FID} is correctly deemed to have a model order of 7, while the other
